...
import {
    Search, Star, Filter, MapPin, Building,
    ArrowRight, Activity, BrainCircuit, Fingerprint,
    Zap, Sliders, LayoutGrid
} from 'lucide-react';
...
export default function HotelsPage() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/hotels?search=${search}&minRating=${minRating}&maxPrice=${maxPrice}`)
            .then(res => res.json())
            .then(d => {
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-[#F5F5F7] font-sans selection:bg-emerald-500 selection:text-white pb-32 overflow-hidden">
            ...
            <div className="relative pt-40 pb-20 px-8 md:px-16 overflow-hidden">
                ...
                <div className="absolute top-1/2 left-0 w-[400px] h-[400px] bg-emerald-500/5 rounded-full blur-[100px] -ml-48"></div>
                ...
                <div className="relative z-10 max-w-7xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 bg-white/5 backdrop-blur-3xl p-6 rounded-[3rem] border border-white/10 shadow-2xl">
                        <div className="md:col-span-6 group relative">
                            ...
                            <input
                                type="text"
                                placeholder="Scan for specific nodes (e.g. Iberostar)..."
                                value={search}
            ...
            <div className="max-w-[1800px] mx-auto px-8 md:px-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
                {loading ? (
                    ...
                    data.map((h: any, i) => {
                        let imageUrl = h?.location?.thumbnail?.photoSizeDynamic?.urlTemplate;
                        if (imageUrl) imageUrl = imageUrl.replace(/\?w=\d+&h=\d+/, '?w=800&h=1200');
                        else imageUrl = 'https://placehold.co/800x1200/0a0a0a/10b981?text=Neural+Link+Lost';

                        const price = h?.location?.locationV2?.hotelPriceRanges?.minimum || 'N/A';
                        const rating = h?.location?.reviewSummary?.rating || 'N/A';
                        const name = h?.location?.locationV2?.names?.name || 'Unknown Node';

                        return (
                            <Link
                                href={`/hotels/${h?.location?.locationV2?.locationId}`}
                                key={i}
                            ...
                            >
                                ...
                                <div className="absolute top-8 left-8 flex flex-col gap-2">
                                    ...
                                    <div className="bg-white/5 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-xl font-black text-white/50 text-[10px] uppercase tracking-widest block opacity-0 group-hover:opacity-100 transition-all">
                                        Level: Premium
                                    </div>
                                </div>
                                ...
                                <div className="absolute bottom-0 left-0 w-full p-10 text-white flex flex-col justify-end">
                                    <div className="mb-4 translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                                        <p className="text-[10px] font-black tracking-[0.4em] uppercase text-emerald-400 mb-2">Verified Node</p>
            ...
            {!loading && data.length === 0 && (
                <div className="col-span-full text-center py-40">
                    <BrainCircuit size={80} className="mx-auto text-emerald-500/20 mb-8 animate-pulse" />
                    <p className="text-2xl font-black text-white/30 uppercase tracking-[.5em]">No compatible nodes found.</p>
                    <button onClick={() => { setSearch(''); setMinRating('0'); setMaxPrice('5000'); }} className="mt-8 text-emerald-500 font-bold border-b border-emerald-500 pb-1">Reset Filters</button>
                </div>
            )}
        </div>
    );
